...
def get_scheduler(
    ...
    *,
    ...
    if scheduler_type == "wsd":
        assert num_stable_steps is not None, "num_stable_steps must be specified for wsd scheduler"
...
def get_wsd_schedule_with_warmup(
    ...
    """
    Create a schedule with warmup, stable, and decay phases.
    
    Args:
        optimizer: The optimizer for which to schedule the learning rate.
        num_warmup_steps: The number of steps for the warmup phase.
        num_stable_steps: The number of steps for the stable phase (constant LR).
...
def _get_wsd_schedule_lambda(
    ...
    *,
    ...
    """
    Compute the learning rate multiplier for the WSD schedule.
    
    Phase 1 (Warmup): Linear increase from 0 to 1
    Phase 2 (Stable): Constant at 1
    Phase 3 (Decay): Cosine decay from 1 to min_lr_ratio
    """
    assert 0 < min_lr_ratio <= 1.0, "min_lr_ratio must be in (0,1]"
    assert num_warmup_steps + num_stable_steps <= num_training_steps, \
        "warmup + stable steps must be <= total training steps"
    ...
    progress = float(current_step - num_warmup_steps - num_stable_steps) / float(max(1, decay_steps))
    cosine_decay = 0.5 * (1.0 + math.cos(math.pi * progress))
