"""Triton-optimized operations for POET.

This module contains custom Triton kernels for high-performance POET operations.
These are imported conditionally since Triton may not be available on all systems.
"""
...
try:
    ...
    print("Triton not found. Installing it is recommended for optimal performance. To install: pip install triton")
    HAS_TRITON = False
...
if HAS_TRITON:
    # =============================================================================
    # Cayley Transform Kernels
    # =============================================================================
    
    def cayley_get_configs(pre_hook=None):
        return [
            triton.Config({'BLOCK_ROW': BM, 'BLOCK_COL': BM, "BLOCK_K": BK, "GROUP_SIZE_M": 8},
    ...
    @triton.autotune(configs=cayley_get_configs(), key=['N', 'B'])
    @triton.jit
    ...
    # Custom PyTorch Cayley Operators
    ...
    def cayley_kernel_bwd(Q: torch.Tensor, Q2: torch.Tensor, dY: torch.Tensor) -> torch.Tensor:
        ...
        #      = 2 * (dY + dQ2) + (2 * QT + Q2T) @ dQ2 + (2 * dY + dQ2) @ Q2T
        #   where: dQ2 = QT @ dY + dY @ QT

        N, B, _ = dY.shape

        QT = Q.transpose(-2, -1)
        Q2T = Q2.transpose(-2, -1)
    ...
    def cayley_backward(ctx, dY: torch.Tensor, dQ2) -> torch.Tensor:
    ...
    # =============================================================================
    # Memory-Efficient Chain Layer Operations
    # =============================================================================
    ...
    def chain_layer_checkpoint_q8_backward(ctx, g):
    ...
    def chain_layer_x_checkpoint_mem_o2(*args, **kwargs):
        raise NotImplementedError(
            "Memory-efficient mode requires Triton. "
            "Please install Triton or use mem_efficient_mode=False."
        )
    
    def chain_layer_x_checkpoint_q8(*args, **kwargs):
        raise NotImplementedError(
            "Quantized POET requires Triton. "
            "Please install Triton to use QPOET."
        )
    
    def chain_layer_x_checkpoint_mem_o2_q8(*args, **kwargs):
        raise NotImplementedError(
            "Memory-efficient quantized mode requires Triton. "
            "Please install Triton or use mem_efficient_mode=False."
        )
