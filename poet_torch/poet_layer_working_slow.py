...
def forward_core(
    ...
    x = permute_x(x, perm_in, perm_in_inv)
    y = x @ base_weight.t()
    if base_bias is not None:
        y = y + base_bias
    y = permute_x(y, perm_out_inv, perm_out)
...
class POETLinear(nn.Module):
    ...
    def merge_then_reinitialize(self) -> None:
        ...
        # W = self.weight.detach().clone()  # (out, in)
        # device, dtype = W.device, W.dtype

        # # Identity block stacks for no-op sides
        # I_b = torch.eye(self.block_size, device=device, dtype=dtype)
        # I_out = I_b.unsqueeze(0).repeat(self.r_out, 1, 1)
        # I_in  = I_b.unsqueeze(0).repeat(self.r_in, 1, 1)

        # # We need W_new such that:
        # # W_new^T = P_in^T @ R_in @ P_in @ W^T @ P_out^T @ R_out @ P_out
        # # Equivalently on W (no transpose):
        # # W_new = P_out^T @ R_out^T @ P_out @ W @ P_in^T @ R_in^T @ P_in

        # # Left conjugation: P_out @ R_out @ P_out.t() @ W
        # tmp_out = W.index_select(0, self.perm_out)                      # P_out @ W
        # # tmp = block_diag_lr_matmul(R_out, tmp, I_in)  # R_out^T @ (...)
        # tmp_out = tmp_out.index_select(1, self.perm_out_inv)                # P_out^T @ (...)

        # # Right conjugation: (...) @ P_in @ R_in @ P_in.t()
        # tmp_in = tmp_in.index_select(0, self.perm_in)                 # (...) @ P_in^T
        # # tmp = block_diag_lr_matmul(I_out, tmp, R_in)  # (...) @ R_in^T
        # tmp_in = tmp_in.index_select(1, self.perm_in_inv)                     # (...) @ P_in

        # tmp = block_diag_lr_matmul(R_out, tmp_out, R_in)

        # Commit and reset rotations; keep permutations unchanged
        # self.weight.detach().copy_(tmp)
