...
def forward_core(
    ...
    # B, S, D = x.shape
    # num_blocks = R_in.shape[0]
    # x_shuffled = x.view(B, S, num_blocks, block_size).transpose(-2, -1).reshape(B, S, D)
    # y = chain_layer_x_pytorch(x_shuffled, R_in, base_weight, base_bias, R_out, block_size)
...
class POETLinearMonarch(nn.Module):
    def __init__(self, in_features, out_features, bsz=256, bias=False, device=None, dtype=None, mem_efficient_mode=False):
        ...
        # perm_out = get_shuffle_indices(out_features, bsz)
        def get_shuffle_indices(D, block_size):
            num_blocks = D // block_size
            # 1. Create the identity indices [0, 1, 2, ..., D-1]
            indices = torch.arange(D)
            # 2. Apply the exact same "Stride Permutation" logic
            # view as (N, b) -> transpose -> flatten
            shuffle_indices = indices.view(num_blocks, block_size).transpose(0, 1).reshape(-1)
            return shuffle_indices

        perm_in = get_shuffle_indices(in_features, bsz)
        perm_out = get_shuffle_indices(out_features, bsz)
    ...
    def perform_permutation(self) -> None:
        ...
        W = self.weight
        Wp = W.index_select(0, self.perm_out_inv)
...
class QPOETLinear(nn.Module):
    ...
    def forward(self, x):
        # dequantize to float *and* make it a leaf so autograd gives us grad_w
        ...
        x = forward_core(x, self.oft_R, self.block_size, self.rows, self.cols, 
                    self.perm_in, self.perm_in_inv, self.perm_out, self.perm_out_inv, 
                    self.r_in, self.r_out, float_weight, self.bias, self.mem_efficient_mode)

        return x


def replace_linear_with_poet_monarch(module: nn.Module, block_size: int, init_type: str, device=None, dtype=None, 
...
def check_and_merge_monarch(model: nn.Module, iter_count=0, poet_reset_gap=4):
