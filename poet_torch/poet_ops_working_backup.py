...
def chain_layer_checkpoint_mem_o2_q8_setup_context(ctx, inputs, output):
    x, Rin, W_q, W_scales, W_zeros, group_size, b, Rout, perm_in_inv, perm_in, bsz = inputs
    # _, yb_r = output
    ctx.save_for_backward(x, Rin, Rout, perm_in_inv, perm_in)
    ctx.W_q = W_q
...
def chain_layer_checkpoint_mem_o2_q8_backward(ctx, g: torch.Tensor) -> Tuple[
    Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor],
    Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor],
    Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor],
    Optional[torch.Tensor], Optional[torch.Tensor]
]:
    x, Rin, Rout, perm_in_inv, perm_in = ctx.saved_tensors
    # x = x[..., perm_in_inv]
    W_q = ctx.W_q
    ...
    # Dequantize weight on-the-fly in backward
    ...
    # Return: x, Rin, W_q, W_scales, W_zeros, group_size, b, Rout, perm_in_inv, perm_in, bsz
    return grad_x, grad_Rin, None, None, None, None, None, grad_Rout, None, None, None

@chain_layer_checkpoint_mem_o2_q8.register_fake
def _(x, Rin, W_q, W_scales, W_zeros, group_size, b, Rout, perm_in_inv, perm_in, bsz):
    B, S, Din = x.shape
    # rin = Rin.size(0)
    rout = Rout.size(0)
    N = B * S
...
def chain_layer_checkpoint_mem_o2_setup_context(ctx, inputs, output):
    x, Rin, W, b, Rout, perm_in_inv, perm_in, bsz = inputs
    # _, yb_r = output
    ctx.save_for_backward(x, Rin, Rout, perm_in_inv, perm_in)
    ctx.W = W
    ctx.b = b
...
def chain_layer_checkpoint_mem_o2_backward(ctx, g: torch.Tensor) -> Tuple[
    Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor],
    Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor],
    Optional[torch.Tensor], Optional[torch.Tensor]
]:
    ...
    return grad_x, grad_Rin, None, None, grad_Rout, None, None, None

@chain_layer_checkpoint_mem_o2.register_fake
def _(x, Rin, W, b, Rout, perm_in_inv, perm_in, bsz):
    B, S, Din = x.shape
    # rin = Rin.size(0)
    rout = Rout.size(0)
    N = B * S
